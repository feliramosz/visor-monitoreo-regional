document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ===============================================
    // ===== LÓGICA PARA MONITOREO X / TWITTER =====
    // ===============================================

    const twitterSection = document.getElementById('monitoreo-twitter');
    const twitterAccountInput = document.getElementById('twitterAccountInput');
    const addTwitterAccountBtn = document.getElementById('addTwitterAccountBtn');
    const twitterAccountsList = document.getElementById('twitterAccountsList');
    ...
    // Función para renderizar la lista de cuentas
    function renderTwitterAccounts() {
        twitterAccountsList.innerHTML = '';
        if (twitterConfig.accounts.length === 0) {
            twitterAccountsList.innerHTML = '<p>No hay cuentas en monitoreo.</p>';
            return;
        }
        twitterConfig.accounts.forEach(account => {
            const accountEl = document.createElement('div');
            accountEl.style = 'display: flex; justify-content: space-between; align-items: center; padding: 8px; border-bottom: 1px solid #ddd;';
            accountEl.innerHTML = `
                <span style="font-weight: bold;">@${account}</span>
                <button type="button" class="remove" data-account="${account}">Eliminar</button>
            `;
            accountEl.querySelector('button').addEventListener('click', () => {
                twitterConfig.accounts = twitterConfig.accounts.filter(acc => acc !== account);
                renderTwitterAccounts();
            });
            twitterAccountsList.appendChild(accountEl);
        });
    }
    ...
    async function loadTwitterConfig() {
        try {
            const response = await fetch('/api/twitter_config', {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (!response.ok) throw new Error('No se pudo cargar la configuración de Twitter.');
            
            const config = await response.json();
            twitterConfig = config;
            
            // Poblar los campos de la interfaz
            apiCallCountEl.textContent = config.monthly_api_calls || 0;
            apiResetDateEl.textContent = config.last_reset_date || 'N/A';
            pollIntervalInput.value = (config.poll_interval_seconds || 600) / 60; // Convertir segundos a minutos
            renderTwitterAccounts();
    ...
    // Event listener para el botón "Añadir Cuenta"
    addTwitterAccountBtn.addEventListener('click', () => {
        const newAccount = twitterAccountInput.value.trim().replace('@', '');
        if (newAccount && !twitterConfig.accounts.includes(newAccount)) {
            twitterConfig.accounts.push(newAccount);
            renderTwitterAccounts();
            twitterAccountInput.value = '';
        }
    });

    // Sobrescribimos el listener del botón "Guardar" para incluir la configuración de Twitter
    saveDataBtn.addEventListener('click', async () => {
        // ... (el código existente para guardar los datos del informe se ejecuta primero)
        
        // Ahora, guardamos la configuración de Twitter
        try {
            const intervalInMinutes = parseInt(pollIntervalInput.value, 10);
            const payload = {
                accounts: twitterConfig.accounts,
                poll_interval_seconds: intervalInMinutes * 60
            };
            ...
            // El mensaje de éxito general ya se muestra, así que no necesitamos uno específico aquí
            // a menos que queramos confirmar explícitamente.
            console.log('Configuración de Twitter guardada exitosamente.');

        } catch (error) {
            showMessage(`Error al guardar configuración de Twitter: ${error.message}`, 'error');
        }
    });

    // Asegurarse de cargar la configuración cuando se hace clic en la pestaña del menú
    document.querySelector('a[data-section="monitoreo-twitter"]').addEventListener('click', loadTwitterConfig);
